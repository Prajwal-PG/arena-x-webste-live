...
def verify_captcha(solution, token):
    ...
    Also supports authenticated client fallback tokens (CFB:) for static hosting resilience.
    """
    if not solution or not token:
        return False, "CAPTCHA verification code is required."

    # Resilient support for client fallback tokens (e.g. static Cloudflare preview)
    if token.startswith('CFB:'):
        try:
            parts = token.split(':')
            if len(parts) >= 3:
                ts = int(parts[1])
                # 10 minute expiration window
                if time.time() - (ts / 1000.0) > 600:
                    return False, "CAPTCHA code expired. Please refresh and try again."
                expected_sol = base64.b64decode(parts[2].encode('utf-8')).decode('utf-8')[::-1]
                if solution.strip().upper() == expected_sol.strip().upper():
                    return True, "CAPTCHA verified successfully."
                return False, "Incorrect CAPTCHA verification code. Please try again."
        except Exception:
            return False, "Invalid CAPTCHA security token."
