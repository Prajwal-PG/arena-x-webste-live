...
- [ ] Old development passwords and admin keys (`arenax2026`) are completely abandoned.
...
- [ ] Production Gmail 16-character App Password configured in `.env`.
- [ ] `.env` file permissions set to `chmod 640` and owned by `root:www-data`.
- [ ] `.env.example` contains zero real credentials or real email addresses.
...
- [ ] `ALLOWED_ORIGINS` configured strictly to actual domain (no wildcard `*`).
